...
impl Registry {
    ...
    pub fn register(&mut self, model: Model) -> Result<(), RusdooError> {
        ...
        let mut write_hooks: Vec<crate::hooks::Hook> = Vec::new();
        ...
        for parent in meta.inherit.iter().rev() {
            ...
            write_hooks.extend(parent_model.hooks().iter().cloned());
        ...
        let merged = merged
            .with_unlink_hooks(hooks)
            .with_hooks(write_hooks)
            .with_onchanges(onchanges);
        self.models
